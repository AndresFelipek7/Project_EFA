		...
		case true:
			...
			if($resultado_Insercion_datos_Basicos){
				...
				if($count >=1){
					...
					$query2 = "INSERT INTO usuario ( id_datos , id_rol , nombre_usuario , pass_usuario ) VALUES ( '$valor_dato' , '2' , '$traer_Valores[8]' , '$traer_Valores[9]')";
					$resultado1 = $conexion -> query($query2);

					echo ($resultado1) ? "<script> alert_dinamic_create('Administrador','administrador.php'); </script>" : "<script> alert_dinamic_mistake('Administrador','administrador.php'); </script>";
				}else {
					echo "<script>  alert_dinamic_mistake('Administrador','administrador.php'); </script>";
				}
			}else{
				echo "<script>  alert_dinamic_mistake('Administrador','administrador.php'); </script>";
			}
		...
		case false:
			...
			if($resultado_Datos_basicos){
				...
				if($count >=1){
					...
					$insercion_Usuario = "INSERT INTO usuario ( id_datos , id_rol , nombre_usuario , pass_usuario ) VALUES ( '$valor_dato' , '2' , '$traer_Valores[8]' , '$traer_Valores[9]')";
					$resultado_Insercion_usuario = $conexion -> query($insercion_Usuario);

					echo ($resultado_Insercion_usuario) ? "<script> alert_dinamic_create('Administrador','administrador.php'); </script>" : "<script> alert_dinamic_mistake('Administrador','administrador.php'); </script>";
				}else {
					echo "<script>  alert_dinamic_mistake('Administrador','administrador.php'); </script>";
				}
			}else{
