<?php
	...
	$query = "SELECT * FROM reporte_usuario_inactivo WHERE documento = '$valor_documento'";
...
<body>
	...
	<div id="logo_empresa">
		...
		<h1 class="titulo">Reporte Evaluador Inactivo con Documento <?php echo $valor_documento; ?></h1>
	</div>
	<center>
		<?php
			foreach ($result as $row ) {
				...
				*/
				...
				//Validar que este vacio o no el campo de correo y dar un mensaje al respecto
				if ($row["correo"] == "") {
					$valor_correo_evaluador = "No se ha ingresado ningun correo";
				}else {
					$valor_correo_evaluador = $row["correo"];
				}
		?>
		...
		<label>Correo :</label> <?php echo $valor_correo_evaluador; ?><br>
