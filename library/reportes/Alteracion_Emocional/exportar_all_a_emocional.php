<?php
	...
	include ("../../conec.php");
	$query = "SELECT * FROM reporte_emocional";
...
<body>
	...
	<div id="logo_empresa">
		...
		<h1 class="titulo">Reporte Alteraciones Emocionales</h1>
	</div>
	
	<table>
		<tr>
			<th>ID Alteracion Emocional</th>
			<th>Nombre Alteracion</th>
			<th>Descripcion Alteracion</th>
		</tr>
		<?php
			//Mostramos los resultado sd el BD con la variable $row que fue asignada por $result
			foreach ($result as $row ) { ?>
		<tr>
			<td><?php echo $row['id_emocional']; ?></td>
			<td><?php echo $row['nombre_emocional']; ?></td>
			<td><?php echo $row['descripcion_emocional']; ?></td>
		</tr>
		<?php
			}
		?>
...
<?php
	...
	$filename = "Reporte Alteracion Emocional.pdf";
